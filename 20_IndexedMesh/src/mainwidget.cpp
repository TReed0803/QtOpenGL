...
void MainWidget::initializeGL()
{
  ...
  // Application-specific initialization
  {
    ...
    p.m_matrixBlock.allocate(sizeof(GLfloat) * 16 * 3); // 3 * mat4
    p.m_program->uniformBlockBinding("GlobalMatrices", p.m_matrixBlock);

    // Cache Uniform Locations
    p.u_modelToWorld = p.m_program->uniformLocation("uModelToWorld");
    ...
    for (int level = 0; level < 1; ++level)
    {
      for (int deg = 0; deg < 360; deg += 10)
      {
        ...
        trans.setTranslation(cosine * 15, level * 5, sine * 15);
        p.m_instanceGroup->pushTransform(trans);
        ++sg_count;
      }
    }
...
void MainWidget::paintGL()
{
  ...
  OpenGLProfiler::BeginFrame();
  {
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    {
      ...
      p.m_instanceGroup->updateBuffers();
    }
    {
      OpenGLMarkerScoped _("Render Scene Instanced");
      p.m_instanceGroup->draw();
    }
    /*
    {
      OpenGLMarkerScoped _("Render Scene Regular");
      for (int i = 0; i < sg_count; ++i)
      {
        p.m_program->setUniformValue(p.u_modelToWorld, p.m_instanceGroup->transforms()[i].toMatrix());
        p.m_openGLMesh->draw();
      }
    }
    //*/
