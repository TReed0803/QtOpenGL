  ...
  for (int f = 0; f < FresnelCount; ++f)
  {
    ...
    for (int g = 0; g < GeometryCount; ++g)
    {
      ...
      for (int d = 0; d < DistributionCount; ++d)
      {
        ...
        m_regularLight[f][g][d]->addShaderFromSourceFile(QOpenGLShader::Vertex, ":/resources/shaders/lighting/directionLight.vert");
        m_regularLight[f][g][d]->addShaderDefines(defines.c_str());
        m_regularLight[f][g][d]->addShaderFromSourceFile(QOpenGLShader::Fragment, ":/resources/shaders/lighting/directionLight.frag");
