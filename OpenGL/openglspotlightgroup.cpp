  ...
  for (int f = 0; f < FresnelCount; ++f)
  {
    ...
    for (int g = 0; g < GeometryCount; ++g)
    {
      ...
      for (int d = 0; d < DistributionCount; ++d)
      {
        ...
        m_regularLight[f][g][d]->addShaderFromSourceFile(QOpenGLShader::Vertex, ":/resources/shaders/lighting/spotLight.vert");
        m_regularLight[f][g][d]->addShaderDefines(defines.c_str());
        m_regularLight[f][g][d]->addShaderFromSourceFile(QOpenGLShader::Fragment, ":/resources/shaders/lighting/spotLight.frag");
...
void OpenGLSpotLightGroup::translateUniforms(const OpenGLRenderBlock &stats, Byte *data, OpenGLLightGroup::SizeType step, OpenGLLightGroup::ConstLightIterator begin, OpenGLLightGroup::ConstLightIterator end)
{
  ...
  while (begin != end)
  {
    ...
    lightDest->m_exponential  = 120.0f;
    lightDest->m_minFalloff   = 0.0f;
    lightDest->m_nearPlane    = 0.1f;
    data += step;
    ++begin;
