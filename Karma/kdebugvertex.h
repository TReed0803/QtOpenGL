...
public:
  ...
  KDebugVertex(const KVector3D &position, const KColor &normal);
...
inline KDebugVertex::KDebugVertex(const KVector3D &position, const KColor &color) : m_position(position), m_color(color.redF(), color.greenF(), color.blueF()) {}
...
inline KColor KDebugVertex::color() const { return KColor(m_color.x(), m_color.y(), m_color.z()); }
