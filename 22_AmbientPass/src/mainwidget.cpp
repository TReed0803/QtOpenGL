...
void MainWidget::initializeGL()
{
  ...
  // Application-specific initialization
  {
    ...
    p.m_matrixBlock.allocate(sizeof(GLfloat) * 16 * 10 + sizeof(GLfloat) * (4 + 3)); // 6 * mat4
    ...
    p.m_floorInstance->material().setSpecular(0.5f, 0.5f, 0.5f, 0.5f);
...
void MainWidget::paintGL()
{
  ...
  OpenGLProfiler::BeginFrame();
  {
    ...
    p.m_program->bind();
    {
      ...
      p.m_instanceGroup->update(p.m_projection, p.m_camera.toMatrix(), p.m_cameraPrev.toMatrix());
      p.m_floorGroup->update(p.m_projection, p.m_camera.toMatrix(), p.m_cameraPrev.toMatrix());
    }
    {
...
void MainWidget::updateEvent(KUpdateEvent *event)
{
  ...
  {
    p.m_buffer = (DeferredData)AmbientPass;
  }
